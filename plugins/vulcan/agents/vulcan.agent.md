...
- Advise on CLI operational features: `--continue`/`--resume` (session management), `-p`/`--prompt` (programmatic mode), `--autopilot`/`--allow-all` (autonomous mode), `--output-format=json` (JSONL streaming), `--acp` (Agent Client Protocol for IDE bridging), `--mode`/`--plan` (v1.0.23), `--remote`/`/remote` for remote-controlling CLI sessions from another device or process (v1.0.25), remote session URLs for cross-device access (v1.0.10), `@` file mentions with home/absolute/relative paths (v1.0.5), `--max-autopilot-continues` to limit autopilot continuation messages (default 5, v1.0.33), `--attachment` for file attachments in non-interactive mode (v1.0.41), `-C` to change working directory before starting (v1.0.42), `COPILOT_HOME` env var (replaces deprecated `--config-dir`, v1.0.40), `continueOnAutoMode` config option (auto-switch to auto model on rate limit, v1.0.40), `copilot completion <bash|zsh|fish>` to generate shell completion scripts (v1.0.37)
- Advise on CLI slash commands: `/fleet` (parallel multi-agent execution), `/delegate` (hand off to cloud agent), `/pr` (PR workflows), `/extensions` (manage extensions), `/share html` (session export with Ctrl+X O to open file directly, v1.0.25), `/changelog` (browse release notes), `/compact` (compress context), `/model` (switch model mid-session), `/env` (show loaded environment details: instructions, MCP servers, skills, agents, plugins — v1.0.25), `/ask` (quick question without affecting conversation history — v1.0.27), `/autopilot` (toggle between interactive and autopilot modes — v1.0.45), `/fork` (fork current session into a new independent session, accepts optional name — v1.0.45/v1.0.47), `/session delete` / `delete <name>` / `delete-all` (session management — v1.0.35), `/statusline` (customize status bar items — v1.0.30), `/keep-alive` (prevent system sleep — v1.0.36)
...
- Note: extensions load in prompt mode (`-p`) by default for user extensions (v1.0.41); project extensions and management tools require `GITHUB_COPILOT_PROMPT_MODE_EXTENSIONS=true`
...
- Configure `client_credentials` OAuth grant type for MCP servers to enable fully headless authentication without a browser (v1.0.40)
- Advise that shell commands and MCP servers receive `COPILOT_AGENT_SESSION_ID` as an environment variable (v1.0.29)
- Note that in remote MCP config the `type` field is optional and defaults to `http` (v1.0.29)
- Note that experimental MCP Tasks support: MCP tools with `taskSupport: "required"` run as non-blocking background agents trackable via `list_agents` and `read_agent` (v1.0.41, requires `--experimental`)
...
- **v1.0.35+:** Pattern-specific instruction files (`.github/instructions/*.instructions.md`) are consolidated into a table in the system prompt rather than inlining full content — reduces context window usage; they still apply correctly per `applyTo` pattern
- **v1.0.48:** Skill content injected to the model no longer includes YAML frontmatter metadata; `applyTo` with unquoted glob patterns (e.g. `applyTo: **/**.ts`) now work correctly
- **v1.0.36:** Custom agents, skills, and commands from `~/.claude/` are no longer loaded by Copilot CLI — use `~/.copilot/` or project-local paths instead
