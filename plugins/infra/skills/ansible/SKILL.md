...
3. **Makefile Integration**: A `Makefile` at the project root is MANDATORY and must follow the `makefile` skill targets (`fmt`, `lint`, `test`, `check`, `qa`).
4. **Lint & Format**: Always run `ansible-lint` and `uvx yamllint`. These should be integrated into the `make lint` and/or Molecule `lint` block.
...
Use the latest stable images as documented in [molecule.md](references/molecule.md):
...
2.  **Lint/Format**: `make lint` (Ensure `ansible-lint` and `uvx yamllint` are called).
...
## References
For detailed configuration examples, command syntax, and troubleshooting, refer to the [Molecule 101 Guide](references/molecule.md).
