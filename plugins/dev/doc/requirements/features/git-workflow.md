...
- AC-008: The branch-first guard MUST fall back to `CWD` when the target file's parent directory does not yet exist, so creating files in new subdirectories is still blocked on `main`/`master`
...
- Must not block reads (`view`, `grep`, `glob`) — read access is always allowed
